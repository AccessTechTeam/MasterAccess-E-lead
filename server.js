...
async function main() {
  try {
    ...
    app.get("/verification-campagne", authMiddleware, async (req, res) => {
      const user = req.user.userId;
      try {
        const activeCampagne = await NameListProspet.findOne({
          active: true,
          completed: false,
          userId: user,
        });
        if (activeCampagne) {
          res.json({
            messages:
              "Attendez que la campagne soit terminée pour pouvoir lancer une nouvelle.",
            active: true,
        ...
        res.json({
          messages:
            "Une erreur s'est produite lors de la vérification de la campagne.",
        });
      }
    });
    ...
    app.post("/lol", authMiddleware, async (req, res) => {
      ...
      try {
        ...
        const existingLinks = await Promise.all(
          documents.map((doc) => Links.findOne({ url: doc.url }))
        );

        // Filter out documents that don't already exist in the database
        const newDocuments = documents.filter(
          (doc, index) => !existingLinks[index]
        );
    ...
    cron.schedule("57 14 * * *", async () => {
      ...
      const usersWithActiveCampaigns = await Model.find()
        .where("_id")
        .in(userIds);
      for (const [index, user] of usersWithActiveCampaigns.entries()) {
        ...
        if (linksInBdd.length > 0) {
          console.log("TEST");
          const campagnes = await NameListProspet.find({
            active: true,
            userId: userId,
          });
        ...
        for (const link of linksInBdd) {
          ...
          const namelist = await NameListProspet.findOne({
            _id: link.nameListProspectId,
          });
          ...
          if (status.status === "Attente") {
            ...
            (link.traited = true), (link.Toforce = false);
            //PLACER DANS KSPR
            new klist({
              linkId: link._id,
              user: link.user,
              ...
              .save()
              .then((result) =>
                console.log("Le prospect n'a pas pu etre ajouté !", result)
              )
    ...
    cron.schedule("47 10 * * *", async () => {
      try {
        ...
        for (const [index, user] of usersWithActiveCampaigns.entries()) {
          const campagneActive = campagnes.find(
            (camp) => camp.userId === user._id
          );
          const userId = user._id;
          const token = await Li_at.findOne({ user: userId });
          //AJOUT D'UN DELAI
          ...
          for (const link of linksInBdd) {
            const LinksUrlBdd = link.url;
            const cleanedLinksInBddUrl = LinksUrlBdd.replace(
            ...
            const invitationExists = invitationsOnHold.some(
              (invitation) => invitation.link === linksInBddUrlWithoutQuery
            );
            ...
            if (invitationExists) {
              ...
              if (!existingEntry) {
                new personInvitationPendingSchema({
                  ...
                  .save()
                  .then((result) =>
                    console.log("ajouté aux personnes en attente", result)
                  )
                  .catch((err) => console.log(err));
              }
            } else {
              // SI UN DES LIENS NE CORRESPOND PAS, LE SUPPRIMER DE LA BDD DES INVITATIONS EN ATTENTE
              personInvitationPendingSchema
                .deleteOne({ user: userId, personneId: link._id })
                .then((result) => {
              ...
              if (invitation === true) {
                ...
                const delayTime =
                  ...
                  minDelay;
                await delay(delayTime);
                let messageEnvoye = await verifyInvitationStatus(
                  token.li_at,
                  LinksUrlBdd,
                  prenom
                );
                ...
                if (messageEnvoye === true) {
                  ...
                  SuivisReponse.findOne({ user: userId, personneId: link._id })
                    .then((existingEntry) => {
                      if (existingEntry) {
                        ...
                        SuivisReponse({
                          ...
                          .then((result) => {
                            console.log(
                              "Enregistré dans la base de données de suivi de réponses :",
                              result
                            );
                          })
                          .catch((err) => console.log("Erreur :", err));
                    ...
                    .catch((err) =>
                      console.log(
                        "Erreur lors de la recherche dans la base de données de suivi de réponses :",
                        err
                      )
                    );
                }
                  ...
                  .then((existingPerson) => {
                    if (existingPerson) {
                      ...
                      new klist({
                        ...
                        .then((result) =>
                          console.log(
                            "Cette personne a refusé la demande de connexion :",
                            result
                          )
                        )
                        .catch((err) => console.log("Erreur :", err));
      ...
      try {
        ...
        for (const [index, user] of usersWithActiveCampaigns.entries()) {
          ...
          for (const link of linksInBdd) {
            ...
            const nom = nomComplet[1];
            const prenom = nomComplet[0];
            await SuivisReponse.findOne({
              user: userId,
              personneId: link._id,
              campagneId: campagnes[index]._id,
            })
              .then(async (responseSuivi) => {
                const dateEnvoiPremierMessage = new Date(
                  responseSuivi.dateEnvoiMessage
                );
                const currentDate = new Date();
                const timeDifference = Math.abs(
                  currentDate.getTime() - dateEnvoiPremierMessage.getTime()
                );
                ...
                let response1 = await checkResponsePerson(
                  token.li_at,
                  link.url
                );
                ...
                if (response1 === true) {
                  ...
                  await SuivisReponse.findOneAndDelete({
                    personneId: link._id,
                    user: userId,
                    campagneId: campagnes[index]._id,
                  })
                    .then((result) =>
                      console.log("Supprimé avec Succes ! ", result)
                    )
                    .catch((err) => {
                      console.log(err);
                    });
                ...
                //ICI !
                  ...
                  console.log(
                    "le prospect n'a pas repondu, Envoie du deuxieme message en cours... "
                  );
                  //AJOUT D'UN DELAI
                  const minDelay = 3000;
                  const maxDelay = 10000;
                  const delayTime =
                    ...
                    minDelay;
                  await delay(delayTime);
                  const secondMessage = await sendSecondMsg(
                    token.li_at,
                    link.url,
                    prenom
                  );
                  if (secondMessage === true) {
                    ...
                    await SuivisReponse.findOneAndUpdate(
                      {
                        personneId: link._id,
                        user: userId,
                        campagneId: campagnes[index]._id,
                      ...
                      .then((existingPerson) => {
                        if (existingPerson) {
                          ...
                          new klist({
                            ...
                            .then((result) =>
                              console.log(
                                "Le deuxieme message n'a pas pu etre envoyé a la personne",
                                result
                              )
                            )
                            .catch((err) => console.log("Erreur :", err));
        ...
        for (const [index, user] of usersWithActiveCampaigns.entries()) {
          ...
          for (const link of linksInBdd) {
            ...
            await SuivisReponse.findOne({
              ...
              .then(async (responseSuivi) => {
                const dateEnvoiPremierMessage = new Date(
                  responseSuivi.dateDeuxiemeMessage
                );
                const currentDate = new Date();
                const timeDifference = Math.abs(
                  currentDate.getTime() - dateEnvoiPremierMessage.getTime()
                ...
                if (response2 === true) {
                  await Links.findOneAndUpdate(
                    ...
                    .then((resultat) => {
                      console.log("champ mis a jour !", resultat);
                    })
                    .catch((err) => console.log(err));
                  ...
                  await SuivisReponse.findOneAndDelete({
                    ...
                    .then((resultat) => {
                      console.log("supprimé avec succes !", resultat);
                    })
                    .catch((err) => {
                      console.log(err);
                    });
          ...
          for (const link of linksInBdd) {
            ...
            personInvitationPendingSchema
              ...
              .then(async (invitePending) => {
                ...
                const currentDate = new Date();
                const timeDifference = Math.abs(
                  currentDate.getTime() - dateAjout.getTime()
                );
                const daysDifference = Math.ceil(
                  timeDifference / (1000 * 3600 * 24)
                );
                ...
                if (daysDifference >= 2) {
                  personInvitationPendingSchema
                    ...
                    .then((deleted) =>
                      console.log("supprimé avec succes !", deleted)
                    );
    ...
    cron.schedule("10 10 * * *", async () => {
      ...
      if (filteredLinks.length > 0) {
        const allLinksSatisfyConditions = filteredLinks.every(
          (link) => link.traited === true
        );
        ...
        if (allLinksSatisfyConditions) {
          ...
          await klist
            .find({ campagneId: campId, user: userIds })
            .then(async (prospects) => {
              if (!prospects) {
                ...
                for (let prospect of prospects) {
                  ...
                  if (sentEmailCount < 40) {
                    ...
                    if (emailProArray.length > 0 && phonesArray.length === 0) {
                        ...
                        if (mailSent === true) {
                          ProsK.findOne({
                            ...
                            .then((existingProspect) => {
                              if (existingProspect) {
                                console.log(
                                  "Prospect déjà existant dans la base de données !"
                                );
                              } else {
                      ...
                      console.log(
                        "pas d'enrichissement pour cette perssonne !"
                      );
                      await klist.deleteOne({ _id: prospect._id });
                    } else {
                      for (let email of emailProArray) {
                        let mailSent = await mailSender(email);
                        if (mailSent === true) {
                          let existingEntry = await ProsK.findOne({
                            email: email,
                          }).exec();
                          if (existingEntry) {
                            console.log(
                              "Entrée déjà existante avec cet email :",
                              existingEntry
                            );
                          } else {
                            new ProsK({
                          ...
                          if (existingEntry) {
                            console.log(
                              "Entrée déjà existante avec ce numéro :",
                              existingEntry
                            );
                          } else {
                ...
                if (prospects.length === 0) {
                  klist
                    .deleteMany({ campagneId: campId, user: userIds })
                    .then(() => {
                    ...
                    .catch((err) =>
                      console.log(
                        "Erreur lors de la mise à jour de NameListProspet :",
                        err
                      )
                    );
                  NameListProspet.findOneAndUpdate(
                    ...
                    .then(async (updatedCampaign) => {
                      console.log(
                        "NameListProspet mis à jour avec succès :",
                        updatedCampaign
                      );
                      const mail = await mailForCampaignFinished(
                        emails,
                        prenom,
                        updatedCampaign.nameListProspect
                      );
                      if (mail === true) {
    ...
    app.get("/invitePending", authMiddleware, (req, res) => {
      const user = req.user.userId;
      NameListProspet.findOne({ userId: user, active: true }).then((result) => {
        console.log(result);
        if (result) {
          personInvitationPendingSchema
            .find({ user, campagneId: result._id })
            .then((data) => {
              res.json({ perssone: data });
            })
            .catch((err) => {
              res.json({ err: err });
            });
    ...
    app.post("/stop-campaign", authMiddleware, async (req, res) => {
      const user = req.user.userId;
      try {
        const campActiv = NameListProspet.findOne({
          userId: user,
          active: true,
          completed: false,
        }).then(async (campagne) => {
          QueueItem.deleteMany({ user, nameListProspectId: campagne._id })
            .then((queus) => {
              console.log("queus deleted successfully ! ", queus);
            })
            .catch((err) => {
              console.log(err);
            });
          ...
          klist
            .deleteMany({ user, campagneId: campagne._id })
            .then((klist) => {
              console.log("klist deleted successfully ! ", klist);
            })
            .catch((err) => {
              console.log(err);
            });
          ...
          SuivisReponse.deleteMany({ user, campagneId: campagne.id })
            .then((suivisRes) => {
              console.log("suivisRes deleted successfully ! ", suivisRes);
            })
            .catch((err) => {
              console.log(err);
            });
          ...
          personInvitationPendingSchema
            .deleteMany({ user, campagneId: campagne.id })
            .then((invitePend) => {
              console.log("invitePend deleted successfully ! ", invitePend);
            })
            .catch((err) => {
              console.log(err);
            });
          ...
          return res
            .status(200)
            .json({ message: "La campagne a été arrêté avec succès." });
        });
      } catch (error) {
        console.error(error);
        return res
          .status(500)
          .json({
            message: "Une erreur s'est produite lors de l'arrêt du processus.",
          });
      }
    });
    ...
    app.get("/all-campaigns-finished", authMiddleware, (req, res) => {
      ...
      NameListProspet.find({ active: false, completed: true, userId: user })
        .then((result) => {
          ...
          Links.find({
            user: user,
            nameListProspectId: { $in: nameListProspectIds },
          })

            .then((data) => {
              ProsK.find({
                campagneId: { $in: nameListProspectIds },
                userId: user,
              })
                .then((prosK) => {
    ...
    app.get("/addingProspectinCampagne", authMiddleware, async (req, res) => {
      ...
      Li_at.findOne({ user })
        .then(async (result) => {
          const adding = await addProspect(
            result.li_at,
            lien_linkedin,
            message
          );
          ...
          InvitationAll.findOneAndUpdate(
            ...
            .catch((err) => {
              if (err.code === 11000) {
                // 11000 est le code d'erreur pour les violations d'unicité dans MongoDB
    ...
    app.get("/enrichissements", authMiddleware, async (req, res) => {
      Li_at.findOne({ user: req.user.userId }).then(async (data) => {
    ...
    app.get(
      "/campagnes_mailing/:campaignId/:persons",
      authMiddleware,
      (req, res) => {
        ...
        ProspectRechecheModels.find({
          nameListProspect: campagneId,
          _id: { $in: personsArray },
        })
          .then((result) => {
            res.json(result);
    ...
    app.post("/li_at", (req, res) => {
      ...
      newLi_at
        ...
        .then((result) => {
          console.log(
            "li_at enregistré avec succès dans la base de données Li_at"
          );
    ...
    app.get("/recup", authMiddleware, (req, res) => {
      Li_at.findOne({ user: req.user.userId }).then(async (result) => {
      ...
      Li_at.findById(li_atId)
        ...
        .then((foundLi_at) => {
          ...
          if (foundLi_at.user.equals(userIdObject)) {
            ...
            return res
              .status(403)
              .send("Vous n'êtes pas autorisé à modifier ce li_at");
          }
        })
        .catch((err) => {
          console.log(err);
          res
            .status(500)
            .send("Erreur serveur lors de la sauvegarde de li_at modifié");
        });
    });
      ...
      Li_at.findOne({ user: req.user.userId })
        .then(async (result) => {
          ...
          evolutionLinkedIn
            ...
            .then((result) => {
              console.log(
                "EvolutionLinkedIn enregistré avec succès dans la base de données"
              );
    ...
    app.get("/getRelationNumb", authMiddleware, async (req, res, next) => {
      Li_at.findOne({ user: req.user.userId })
        .then(async (result) => {
          ...
          evolutionDesRelation
            .save()
            .then((result) => {
              console.log(
                "evolution Des Relation enregistré avec succès dans la base de données"
              );
      ...
      Li_at.findOne({ user: req.user.userId })
        .then(async (result) => {
          ...
          EvolutionDesInvitation.save()
            .then((result) => {
              console.log(
                "EvolutionDesInvitation enregistré avec succès dans la base de données"
              );
    ...
    app.get("/dataForChart", authMiddleware, (req, res) => {
      const user = req.user.userId;
      EvolutionLinkedIn.find(
        { userId: user },
        { enAttente: 1, Relation: 1, VuesDuProfil: 1, _id: 0 }
      )
        .then((data) => {
    ...
    app.post("/inscription", async (req, res) => {
      const { nom, username, email, password, repetedPassword, photo } =
        req.body;
      ...
      new UserInscription(data)
        .save()
        .then((user) => {
          const token = jwt.sign({ userId: user._id, email }, keySec, {
            expiresIn: "12h",
          });
          user.token = token;
    ...
    app.post("/connexion", async (req, res) => {
      ...
      try {
        ...
        const token = jwt.sign({ userId: user.id, email: email }, keySec, {
          expiresIn: "2h",
        });
    ...
    app.get("/campagneElement/:campaignId", authMiddleware, (req, res) => {
      ...
      NameListProspet.findOne({ _id: idOfCampaign, userId: user }).then(
        (data) => {
          const inQueueTruePromise = Links.find({
            inQueue: true,
            nameListProspectId: idOfCampaign,
            user,
          });
          const inQueueFalsePromise = Links.find({
            inQueue: false,
            nameListProspectId: idOfCampaign,
            user,
          });
          ...
          Promise.all([inQueueTruePromise, inQueueFalsePromise, linksData])
            .then(([inQueueTrueResult, inQueueFalseResult, linksData]) => {
              res.json({
                results: {
                  inQueueTrue: inQueueTrueResult,
                  inQueueFalse: inQueueFalseResult,
                  prospects: linksData,
                },
                data: data,
              });
    ...
    app.delete(
      "/deleteProspectfromProcess/:prospectId",
      authMiddleware,
      (req, res) => {
        const user = req.user.userId;
        const prospectId = req.params.prospectId;

        Links.findOneAndDelete({
          processed: true,
          inQueue: false,
          _id: prospectId,
          user,
          ...
          .then((result) =>
            res.json({ message: "supprimé avec success ! ", result })
          )
    ...
    app.put("/nameListProspect/:id", authMiddleware, async (req, res) => {
      try {
        ...
        const result = await NameListProspet.updateOne(
          { _id: listeID },
          { nameListProspect: nameListProspect }
        );
        res.json(result);
    ...
    app.get("/ProspectRechecheModels/:id", authMiddleware, async (req, res) => {
      try {
        ...
        const result = await ProspectRechecheModels.find({
          nameListProspect: listeID,
        });
...
async function initializeQueue(user) {
    ...
    const result = await NameListProspet.findOne({
      active: true,
      userId: user,
    });
    if (result) {
